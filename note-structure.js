...
// function handleCheckboxAdding(notebody) {
...
//     if (e.key === "Enter") {
//       e.preventDefault();
//       handleCheckboxAdding(notebody);
//       setTimeout(() => {
//         const newCheckboxes = notebody.querySelectorAll(".checkbox-text");
...
//       notebody.replaceChild(paragraph, checkboxDiv);
...
function handleCheckboxAdding(notebody) {
  ...
  const checkboxDiv = document.createElement("div");
  const textspan = document.createElement("span");
  textspan.contentEditable = note.classList.contains("expanded"); // Set based on note state
  textspan.className = "checkbox-text";
  ...
  textspan.addEventListener("keydown", function (e) {
    if (e.key === "Enter") {
      e.preventDefault();
      handleCheckboxAdding(notebody);
      // Get the newly created checkbox's span and focus it
      setTimeout(() => {
        const newCheckboxes = notebody.querySelectorAll(".checkbox-text");
        if (newCheckboxes.length > 0) {
          const lastCheckbox = newCheckboxes[newCheckboxes.length - 1];
          lastCheckbox.focus();
        }
      }, 0);
    }
